...
import {
  ...
  IonIcon, IonModal, IonDatetime, IonGrid, IonRow, IonCol, IonCard
...
export class CreatePlayerPage implements OnInit {
  ...
  async createMap() {
    ...
    this.newMap = await GoogleMap.create({
      ...
      config: { center: { lat: latitude, lng: longitude }, zoom: 5 }
    });
  ...
  async createPlayer() {
    ...
    const confirmed = await this.alertService.showConfirmation(
      'Crear jugador',
      `¿Estás seguro de que quieres registrar a ${this.playerForm.value.name}?`
    );
    if (!confirmed) return;
    this.playerService.createPlayer(this.playerForm.value).subscribe({
      ...
      error: () => {
        this.toastService.showToast('Error al crear el jugador', 'error');
      }
    });
  }
}
